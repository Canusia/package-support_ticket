...
def tickets(request):
    """
    Return tickets created by student — table data served via scoped DRF endpoint.
    """
    return render(request, 'support_ticket/student/index.html', {
        'page_title': 'Support Requests',
        'api_url': '/api/v1/support-ticket-student/?format=datatables',
        'urls': {
            'add_new': 'student_support_ticket:add_new',
            'details': 'student_support_ticket:details',
        },
        'menu': draw_menu(STUDENT_MENU, 'support', '', 'student'),
        'can_start': STS.can_start('student'),
    })
...
def add_new(request):
    """
    Create a new support ticket for the student. Gated by STS.can_start('student').
    """
    if not STS.can_start('student'):
        messages.add_message(
            request, messages.ERROR,
            'New support requests are currently disabled.',
            'list-group-item-danger')
        return redirect('student_support_ticket:requests')

    form = SupportTicketForm('student')
    if request.method == 'POST':
        form = SupportTicketForm('student', request.POST, request.FILES)
        if form.is_valid():
    ...
    return render(request, 'support_ticket/student/add_new.html', {
        'base_template': 'cis/logged-base.html',
        'form': form,
        'page_title': 'Submit New Support Request',
        'labels': {'all_items': 'All Requests'},
        'urls': {'all_items': 'student_support_ticket:requests'},
        'menu': draw_menu(STUDENT_MENU, 'support', '', 'student'),
    })
...
def details(request, record_id):
    """
    Show details for a ticket submitted by the current student (IDOR guard).
    """
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.add_message(
                ...
                'list-group-item-success')
            return redirect('student_support_ticket:details', record_id=record_id)
    else:
    ...
    return render(request, 'support_ticket/student/details.html', {
        'form': form,
